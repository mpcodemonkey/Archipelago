...
## What does another world's item look like in Castlevania: Legacy of Darkness?

An item belonging to another world will show up as that item if it's from another Castlevania: Legacy of Darkness world, or one of two
...
The groups you can use for Castlevania: Legacy of Darkness are `bomb`, `ingredient`, and `crest`. Both of the first two will
hint randomly for either a Magical Nitro or Mandragora, and the third will hint for one of the Crest Halves.

## What are the location name groups?
In Castlevania: Legacy of Darkness, every location that is specific to a stage is part of a location group under that stage's
name. So if you want to exclude all of, say, Art Tower, you can do so by just excluding "Art Tower" as a whole.

## I'm stuck and/or I can't find this hinted location...is there a map tracker?
At the moment, no map tracker exists. [Here](https://github.com/ArchipelagoMW/Archipelago/tree/main/worlds/cvlod/docs/obscure_checks.md)
...
## On the character select screen, why does the highlighter not show what character is behind it?
This is an issue that existed with Castlevania: Leagcy of Darkness on mupen64plus way back in 2017, and BizHawk never updated their
mupen64plus core since it was fixed way back then. This is a Castlevania: Leagcy of Darkness in general problem that happens even with the
vanilla ROM, so there's not much that can done about it besides opening an issue to them (which [has been done](https://github.com/TASEmulators/BizHawk/issues/3671))
